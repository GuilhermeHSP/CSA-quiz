...
export default function ServiceNowCSAQuiz() {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      const response = await fetch('/questions.json') // certifique-se que está em public/
  ...
  const handleAnswerSelection = (selectedOption: string) => {
    ...
    if (selectedOption === shuffledQuestions[currentQuestionIndex].correctAnswer) {
      setScore(score + 1)
    }
  }
